      ...
      */
      ...
      while(pArvore->pNoRaiz->ModoVisita != ARV_ModoDePai)
      {
         ...
         while(ARV_IrNoEsquerda(pArvore) == ARV_CondRetOK)
         {
            ARV_IrPai(pArvore);
            ARV_ObterValorCorr(pArvore,&valor);
            retorno = ConcatNo(Str," %c (",valor);
            if(retorno != ARV_CondRetOK)
               return retorno;
         ...
         if(!NoDir && !NoEsq)
         {
            ...
            /* Caso o no corrente seja o nó a esquerda do seu pai */
...
/***********************************************************************
*
*  $FC Função: ARV Concatenar nó à string de exibição
*
*  $EP Parâmetros
*     $P ValorParm - valor a ser armazenado no novo nó
*
...
***********************************************************************/
   ...
   ARV_tpCondRet ConcatNo( char *Str, char *StrNo, char valor )
   {
      char temp[6];
      sprintf(temp,StrNo,valor);
      if(strlen(temp) + strlen(Str) >= DIM_STR_BUFFER)
      {
         strcpy(Str,"");
         return ARV_CondRetBufferOverflow;
      }
      strcat(Str,temp);
      return ARV_CondRetOK;
   } /* Fim função: ARV Criar nó da árvore */
